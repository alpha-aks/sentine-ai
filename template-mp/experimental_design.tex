...
\begin{itemize}
    \item A per-agent smoothed signal value $\hat{s}_i$, updated via EMA (Equation \ref{eq:ema}) whenever a ViolationEvent is received from agent $i$.
    \item A composite risk score $r$, computed as the weighted sum of agent signals.
    \item An alert level, derived from the composite risk score via the threshold function (Equation \ref{eq:tiers}).
    \item A list of recent ViolationEvents, used for XAI rationale assembly.
    \item A complete alert history for the session.
\end{itemize}

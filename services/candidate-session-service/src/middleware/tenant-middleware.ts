...
export function tenantGuard(paramName: string = 'institutionId') {
  return (req: TenantRequest, res: Response, next: NextFunction): void => {
    ...
    // Admins and compliance officers can cross tenant boundaries
    ...
    if (!actorTenantId || (targetInstitutionId && targetInstitutionId !== actorTenantId)) {
      res.status(403).json({
        success: false,
        error: {
          code: 'SESSION_CROSS_TENANT_FORBIDDEN',
          message: 'Access denied: Cannot access sessions outside your institution',
          details: []
        },

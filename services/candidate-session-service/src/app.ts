...
export function createApp(controller?: SessionController): Express {
  ...
  app.get('/health', (_req: Request, res: Response) => {
    res.status(200).json({
      status: 'UP',
      service: 'sentinel-ai-candidate-session-service',
      timestamp: new Date().toISOString()
    });
  });

  // Mount routes
  const router = createSessionRouter(controller);

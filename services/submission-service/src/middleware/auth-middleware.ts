...
export function authenticateJwt(req: AuthenticatedRequest, res: Response, next: NextFunction): void {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    res.status(401).json({
      success: false,
      error: { code: 'SUBMISSION_UNAUTHORIZED', message: 'Authentication token missing or invalid', details: [] },
  ...
  try {
    ...
    res.status(401).json({
      success: false,
      error: { code: 'SUBMISSION_INVALID_TOKEN', message: err.message || 'Token verification failed', details: [] },
      meta: { requestId: `req_${Date.now()}`, timestamp: new Date().toISOString(), path: req.originalUrl }
    });
  }
}
...
export function requireRole(allowedRoles: UserRole[]) {
  return (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
    if (!req.user || !req.user.role) {
      res.status(401).json({
        success: false,
        error: { code: 'SUBMISSION_UNAUTHORIZED', message: 'User authentication required', details: [] },
        meta: { requestId: `req_${Date.now()}`, timestamp: new Date().toISOString(), path: req.originalUrl }
      });
      return;
    }
...
export function rateLimiter(maxRequests: number = 60, windowMs: number = 60000) {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (record.count > maxRequests) {
      res.status(429).json({
        success: false,
        error: { code: 'SUBMISSION_RATE_LIMITED', message: 'Too many requests. Please slow down.', details: [] },
        meta: { requestId: `req_${Date.now()}`, timestamp: new Date().toISOString(), path: req.originalUrl }
      });
      return;
    }
